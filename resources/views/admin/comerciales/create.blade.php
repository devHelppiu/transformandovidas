    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('admin.comerciales.store') }}">
                    @csrf
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select id="comision_tipo" name="comision_tipo" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                    ...
                                    <option value="porcentaje" {{ old('comision_tipo') === 'porcentaje' ? 'selected' : '' }}>Porcentaje</option>
                                    <option value="fijo" {{ old('comision_tipo') === 'fijo' ? 'selected' : '' }}>Fijo por ticket</option>
                                    <option value="meta" {{ old('comision_tipo') === 'meta' ? 'selected' : '' }}>Meta (pendiente)</option>
                                </select>
                                <x-input-error :messages="$errors->get('comision_tipo')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="comision_valor" value="Valor comisión" />
                                <x-text-input id="comision_valor" name="comision_valor" type="number" step="0.01" class="mt-1 block w-full" :value="old('comision_valor')" />
                                <x-input-error :messages="$errors->get('comision_valor')" class="mt-2" />
                            </div>
                        </div>

                        <p class="text-sm text-gray-500">El código de referido se generará automáticamente (formato TV-XXXXXX).</p>
